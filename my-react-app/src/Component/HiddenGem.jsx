  ...
  return (
    ...
    <Slider {...settings}>
      ...
      {data.map((item, index) => (
        ...
        <div key={index} >
               <div className='min-w-sm mt-4 mr-2 border-2  hover:shadow-2xl '>
               <div className='py-6 px-5'>
               <a href={item.linkUrl}  target="_blank" rel="noreferrer">
               <img className="w-full h-35 mb-2" src={item.image} alt="hi"  /></a>
              <p className=" font-bold text-black-300 text-xl ">{item.title}</p>
               <p className="mt-4  text-black-400 font-semibold"><del className='mr-2'>{item.price}</del>{item.nprice}</p>
